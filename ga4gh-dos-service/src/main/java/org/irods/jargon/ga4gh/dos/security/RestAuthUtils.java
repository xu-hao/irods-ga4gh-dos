...
import org.irods.jargon.ga4gh.dos.utils.DefaultHttpClientAndContext;
...
public class RestAuthUtils {
	...
	public static IRODSAccount getIRODSAccountFromUserPassword(final String user, final String password,
		...
		log.info("restConfiguration:{}", dosConfiguration);

		AuthScheme authScheme = determineAuthSchemeFromConfig(dosConfiguration);
		...
		try {
			return IRODSAccount.instance(dosConfiguration.getIrodsHost(), dosConfiguration.getPort(), myUser, password,
					"", dosConfiguration.getIrodsZone(), dosConfiguration.getDefaultStorageResource(), authScheme);
	...
	private static AuthScheme determineAuthSchemeFromConfig(final DosConfiguration restConfiguration) {
		return restConfiguration.translateAuthSchemeToEnum();
	}
